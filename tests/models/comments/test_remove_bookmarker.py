...
class TestRemoveBookmarker(BaseTestCase):
    def test_remove_bookmarker(self):
        ...
        # Remove the user from the comment's bookmarkers
        comment.remove_bookmarker(user)

        # Assert that the user is not in the comment's bookmarkers
        self.assertNotIn(user, comment.comment_bookmarkers)